...
async def startupmessage():
    try:
        if BOTLOG:
            Config.CATUBLOGO = await O9937.tgbot.send_file(BOTLOG_CHATID, "
            https://telegra.ph/file/1bb8d03e5402d07c7c407.jpg", caption="♛ ⦙ تـمّ اعـادة تشـغيل\n تليثـون العـرب ✓  :  [ 7.5 ] .\n\n♛ ⦙ للحصول على اوامر السورس\n أرسـل : (  `.اوامري`  ) \n\n♛ ⦙ لمـعرفة كيفية تغير بعض كلايش\n او صور السـورس  أرسـل  :\n (  `.مساعده`  )\n\n♛ ⦙ القناة الرسمية مستقبل  : @O9937\n♛ ⦙ فارات سورس مستقبل  :@O9937 \n♛ ⦙ كلايش مستقبل :  @O9937\n ♛ ⦙التحديثات والاضافات :  @O9937\n",                buttons=[(Button.url("مطور مستقبل الرسمي", "https://t.me/O9937"),)],            )
    except Exception as e:
        LOGS.error(e)
        return None
async def add_bot_to_logger_group(chat_id):
    bot_details = await O9937.tgbot.get_me()
    try:
        await O9937(            functions.messages.AddChatUserRequest(                chat_id=chat_id,                user_id=bot_details.username,                fwd_limit=1000000            )        )
...
async def setup_bot():
    try:
        await O9937.connect()
        config = await O9937(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == O9937.session.server_address:
                if O9937.session.dc_id != option.id:
                    LOGS.warning(                        f"♛ ︙ معرف DC ثابت في الجلسة من {O9937.session.dc_id}"                        f"♛ ︙ يتبع ل {option.id}"                    )
                O9937.session.set_dc(option.id, option.ip_address, option.port)
                O9937.session.save()
                break
        bot_details = await O9937.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        # await O9937.start(bot_token=Config.TG_BOT_USERNAME)
        O9937.me = await O9937.get_me()
        O9937.uid = O9937.tgbot.uid = utils.get_peer_id(O9937.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(O9937.me)
    except Exception as e:
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await O9937.get_entity(BOTLOG_CHATID)
        ...
        _, groupid = await create_supergroup(            "مجموعه بوت مستقبل الخاص بك", O9937, Config.TG_BOT_USERNAME, descript        )
        addgvar("PRIVATE_GROUP_BOT_API_ID", groupid)
        print("♛ ︙ تم إنشاء مجموعة المسـاعدة بنجاح وإضافتها إلى المتغيرات.")
        flag = True
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await O9937.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
        ...
        _, groupid = await create_supergroup(            "كـروب تخزين الخاص", O9937, Config.TG_BOT_USERNAME, descript        )
