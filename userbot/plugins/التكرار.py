...
@O9937.iq_cmd(pattern="تكرار حزمه الملصقات$",)
...
@O9937.iq_cmd(    pattern="تكرار_احرف ([\s\S]*)",)
...
@O9937.iq_cmd(    pattern="تكرار_كلمه ([\s\S]*)",)
async def tmeme(event):
...
@O9937.iq_cmd(pattern=f"{MUQT} ([\s\S]*)",)
async def spammer(event):
    ...
    try:
        ...
        return await edit_delete(            event, "عذرا طريقه كتابة الأمر خطأ - شرح الارسال الوقتي او مؤقت للكروبات هنا : https://t.me/O9937/4483"        )
    cat = input_str[1:]
    try:
        int(cat[0])
    except Exception:
