...
async def do_pm_permit_action(event, chat):  
    ...
    if PM_WARNS[str(chat.id)] >= MAX_FLOOD_IN_PMS:
        ...
        if custompmblock is not None:
            ...
            USER_BOT_WARN_ZERO = f"**  تـم تـحـذيـرڪ مـسـبـقـاً مـن الـتـڪـرار . تـم حـظـرڪ ، لا يـمـڪـنـڪ ازعـاج الـمـالـڪ ! You have been warned in advance of repetitive messages.  I cannot let you bother the owner.!**"
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"𓍹ⵧⵧⵧⵧⵧⵧ Future ⵧⵧⵧⵧⵧⵧ𓍻\n\n❞  هـݪـو٘ {mention} ❝ \nاﻧـا آݪان ﻣـﺷـغٓول ݪاتࢪﺳـݪ ݪي ࢪﺳـآئݪ ڪﺛـيࢪه وآݪآ سيٰتم حٓـظٍـࢪڪ فقـط قـࢦ سـبب مجـيٰـئڪ اوَ حٰٖـآِجٰتـڪِٰ ، عٰٖـنِ٘ـدمـا آﻋـۅد سِ٘أوافـق علـى ﻣحٰٖـآِډثـتِـك  .. Hello.  Now I'm busy. Don't send me a lot of messages. Tell me why you came and what you want.  And when I come back I will agree to talk to you.. ❞ \n ⤶ ❨  `{warns}/{totalwarns}` ❩\n\n𓍹ⵧⵧⵧⵧ Future  ⵧⵧⵧⵧ𓍻"
    else:
        USER_BOT_NO_WARN = f"𓍹ⵧⵧⵧⵧⵧⵧ Future ⵧⵧⵧⵧⵧⵧ𓍻\n\n❞  هـݪـو٘ {mention} ❝ \nاﻧـا آݪان ﻣـﺷـغٓول ݪاتࢪﺳـݪ ݪي ࢪﺳـآئݪ ڪﺛـيࢪه وآݪآ سيٰتم حٓـظٍـࢪڪ فقـط قـࢦ سـبب مجـيٰـئڪ اوَ حٰٖـآِجٰتـڪِٰ ، عٰٖـنِ٘ـدمـا آﻋـۅد سِ٘أوافـق علـى ﻣحٰٖـآِډثـتِـك  .. Hello.  Now I'm busy. Don't send me a lot of messages. Tell me why you came and what you want.  And when I come back I will agree to talk to you.. ❞ \n ⤶ ❨  `{warns}/{totalwarns}` ❩\n\n𓍹ⵧⵧⵧⵧ Future  ⵧⵧⵧⵧ𓍻"
...
async def do_pm_options_action(event, chat):
    ...
    sql.del_collection("pmmessagecache")
    sql.add_collection("pmmessagecache", PMMESSAGE_CACHE, {})
    USER_BOT_WARN_ZERO = f"**  تـم تـحـذيـرڪ مـسـبـقـاً مـن تـڪـرار الـرسـائـل .** \n**   تـم حـظـرڪ مـن الـحـسـاب 🚫.** \n**  لـن اسـتـلـم رسـائـلـڪ الـى ان يـاتـي مـالـڪ الـحـسـاب . You were previously warned not to send messages.  You have been banned from the account. I have not received the messages yet. **"
    await event.reply(USER_BOT_WARN_ZERO)
    await event.client(functions.contacts.BlockRequest(chat.id))
    the_message = f"**  حـمـايـة الـخـاص  (الـبـرايـفـت)  : 📩**\n[{get_display_name(chat)}](tg://user?id={chat.id}) **تـم حـظـر الـمـسـتـخـدم .🚫**\n**  الـسـبـب ~> اسـتـمـر بـالـتـكـرار . dm protection.  User blocked due to spam messages .**"
...
async def do_pm_enquire_action(event, chat):
    ...
    try:
        ...
        PMMESSAGE_CACHE = {}
    if str(chat.id) not in PM_WARNS:
        text = "**  يـرجـى الانـتـظـار لــ حـيـن قرائة رسالتك  . \n    مـالـڪ الـحـسـاب سَــوف يـرد عـلـيـڪ عـنـد اسـتـطـاعـتـه . . \n    يـرجـى عـدم تـڪـرار الـرسـائـل لـتـجـنـب الـحـظـر . Please wait while I read your messages.  Do not repeat messages to avoid ban . **"
        await event.reply(text)
        PM_WARNS[str(chat.id)] = 1
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
async def on_plug_in_callback_query_handler(event):
    ...
    text = f"**حسنا الان بإمكانك اختيار احد الخيارات في الاسفل للتواصل مع :** {mention}.\n**   اختر بهدوء خيار واحد فقط لنعرف سبب قدومك هنا  ⬇️ well now.  You can choose one of the options below to contact the owner.  Please choose only one option ⬇️ **"
    buttons = [
        (Button.inline(text="إسـتـفـسـار  | Inquire about .", data="to_enquire_something"),),
        (Button.inline(text="طـلـب مـعـيـن |  specific request .", data="to_request_something"),),
        (Button.inline(text="الـدردشـة| chat .", data="to_chat_with_my_master"),),
        (Button.inline(text="إزعـاج الـمـالـڪ | Annoy the owner .", data="to_spam_my_master_inbox"),),
    ]
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "**   عـذرا ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك **"
        return await event.answer(text, cache_time=0, alert=True)
    text = "**  حـسـنـاً ، تـم ارسـال طـلـبـڪ بـنـجـاح  . لا تـقـم بـأخـتـيـار خـيـار ثـانـي .**\n**  سَــ يـتـم الـرد عـلـيـڪ عـنـد تَـفَـرُغ الـمـالـڪ . Your request has been sent.  Do not choose another option.  You will be replied to by the owner when he is here. **"
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "**   عـذرا ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك **"
        return await event.answer(text, cache_time=0, alert=True)
    text = "**  حـسـنـاً ، لـقـد قـمـت بـأبـلاغ مـالـڪ الـحـسـاب عـنـدمـا يـصـبـح مـتـصـلا بـالانـتـرنـت**\n** لـڪـن حـالـيـاً لا تـڪـرر الـرسـائـل لـتـجـنـب الـحـظـر Okay. You have notified the owner when he becomes available and he will respond to you.  But now do not repeat messages so as not to be banned **"
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "**   عـذرا ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك **"
        return await event.answer(text, cache_time=0, alert=True)
    text = "**  بـالـطـبـع يـمـكـنـك الـتـحـدث مـع مـالـك الـحـسـاب لـكـن لـيـس الان  🤍\n   نـسـتـطـيـع الـتـكـلـم فـي \n   وقـت اخـر حـالـيـا انـا مـشـغـول قـلـيـلاً  - عـنـد تـفـرغـي سـأكـلـمـك هـذا اكـيــد . Of course you can write to the owner.  But not now.  You can talk to me at another time when I will talk to you without a doubt. **"
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "**   عـذرا ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك **"
        return await event.answer(text, cache_time=0, alert=True)
    text = "**  هـذا تـحـذيـرك الأخـيـر ، ارسـل رسـالـة واحـدة وسـيـتـم حـظـرك تـلـقـائـيـاً . This is the last warning.  Send one more message now and you will be automatically blocked‼️ **"
